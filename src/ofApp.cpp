...
void ofApp::setup(){
    ...
    //add a listener to the save button, so the program knows what to do when it's pressed
    saveButton.addListener(this,&ofApp::saveButtonPressed);
    
    //set up the gui panel
    gui.setup();
    //gui.add(renderToggle.setup("render", false));
    gui.add(saveButton.setup("save"));
    gui.add(color.setup("color",ofColor(100,100,140),ofColor(0,0),ofColor(255,255)));
    
 
    //this shows the gui, false hides the gui
    bHide = true;
...
//    
    ...
    ofSetColor(0);
    ofDrawBitmapString("PDF OUTPUT EXAMPLE", 250, 32);
    //if( pdfRendering ){
        //ofDrawBitmapString("press r to stop pdf multipage rendering", 250, 62);
    //}else{
    // removed from string below: press r  to start pdf multipage rendering\n
    ofDrawBitmapString("press save button to save a single screenshot as pdf to disk", 250, 62);
    //}
...
void ofApp::saveButtonPressed(){
    if(!pdfRendering)oneShot = true;
}

//void ofApp::renderTogglePressed(){
//        pdfRendering = true;
//        if( pdfRendering ){
//            ofSetFrameRate(12);  // so it doesn't generate tons of pages
//            ofBeginSaveScreenAsPDF("recording-"+ofGetTimestampString()+".pdf", true);
